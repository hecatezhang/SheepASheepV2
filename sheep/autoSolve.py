...
def auto_solve(map_data, issort, percent, timeout, threadName):
    ...
    while True:
        ...
        if second%2 == 0:
            ...
            if thread1.is_alive():
                stop_thread(thread1)
                print(("\033[0;33;40m自动求解超时！当前算法有些力不从心，建议放弃挑战并重新开始！\033[0m").encode('utf-8'))
                print("==========================================")
                exit(0)
        time.sleep(1)
        second += 1
        print(("\r\033[0;31;40m%s进度：\033[0m%d/%d"%(threadName, second, timeout)).encode('utf-8'),end="")
    ...
    if result != "牌面无解":
        print(("\033[0;32;40m计算用时：\033[0m\033[0;34;40m {:.2f}\033[0m".format(end_time - start_time)).encode('utf-8'))
        ...
        print("result", json.dumps(result["operations"]))
        # print("当前关卡自动求解步骤已保存到当前路径下 map_data_operations.json 文件！")
        # print("==========================================")
        # post_map_data(result)
    else:
        print(("\033[0;33;40m牌面无解！建议放弃挑战并重新开始！\033[0m").encode('utf-8'))
        print("==========================================")
        #self.post_map_data(map_data)
...
if __name__ == '__main__':
    ...
    if args.issort != "true" and args.issort != "reverse" and args.percent == 0.85:
        ...
        threadName = "自定义模式"
    print(("开始求解，请稍等%d秒..."%args.timeout).encode('utf-8'))
